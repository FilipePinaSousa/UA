...
m, b = linreg(T,np.log(A)) #Gráfico log(Atividade(A)) em função do Tempo(T)
logA = np.log(A)
line = T*m + b     #Gráfico da linha

fig, dx = plt.subplots()
dx.plot(T,logA, "x") #Gráfico dos pontos
dx.plot(T, line, "b") #reta
dx.set_xlabel("Tempo(H)")
dx.set_ylabel("Atividade(mBq)")    #r² = 0.9995011037502572
dx.set_xlim(-10, 390)
dx.set_ylim(-1, 3)
dx.set_title("log(Atividade) em função do tempo")
dx.grid()
plt.show()