...
def fourier(x0,v0,m,k,b,F0,Wf,A,t,n,ti_estac): 
...
def planoInclinado_res_1D(x0,v0,n,dt,c_ar,c_atr,A,m,pot,teta=0):
...
def oscAcop_EulerCromer_FA_1D(x0,v0,x_eq,k,m,t,b,F0,w_f,n,dt):
    ...
    for i in range(n):
        aA[i]=(-k_ext*(xA[i]-xA_eq) - k_int*((xA[i]-xA_eq)-(xB[i]-xB_eq)) - b*vA[i] + F0A*np.cos(w_f_A*t[i]))/mA
        aB[i]=(-k_ext*(xB[i]-xB_eq) + k_int*((xA[i]-xA_eq)-(xB[i]-xB_eq)) - b*vB[i] + F0B*np.cos(w_f_B*t[i]))/mB
...
def oscAcop_EulCrom_Mult_Simp_1D(x0,v0,x_eq,k,m,numCorp,n,dt):
    ...
    for i in range(n):
        ...
        for iC in range(1,numCorp-1):
            a[iC][i]=( -k[iC]*((x[iC]-x_eq[iC])-(x[iC-1]-x_eq[iC-1])) -k[iC+1]*((x[iC]-x_eq[iC])-(x[iC+1]-x_eq[iC+1])) )/m[iC]
...
def oscAcop_EulCrom_Mult_FA_1D(x0,v0,x_eq,k,m,numCorp,t,b,F0,w_f,n,dt):
    ...
    for i in range(n):
        #aceleração dos dois corpos conectados à parede:
        a[0][i]=( -k[0]*(x[0][i]-x_eq[0]) -k[1]*((x[0][i]-x_eq[0])-(x[1][i]-x[1][i])) +F0[0]*np.cos(w_f[0]*t[i])) )/m[0]
        a[-1][i]=( -k[-1]*(x[-1][i]-x_eq[-1]) -k[-2]((x[-1][i]-x_eq[-1])-(x[-2][i]-x_eq[-2])) +F0[-1]*np.cos(w_f[-1]*t[i])) )/m[-1]
        #aceleração dos corpos no meio:
        for iC in range(1,numCorp-1):
            a[iC][i]=( -k[iC]*((x[iC]-x_eq[iC])-(x[iC-1]-x_eq[iC-1])) -k[iC+1]*((x[iC]-x_eq[iC])-(x[iC+1]-x_eq[iC+1])) +F0[iC]*np.cos(w_f[iC]*t[i])) )/m[iC]
